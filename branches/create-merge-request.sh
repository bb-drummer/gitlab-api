...
gitlab_create_merge_request () {
    ...
    # check for private access token
    ...
    echo "API host/URL to operate: ${HOST}";

    # Look which is the default branch
    #TARGET_BRANCH=`curl --silent "${HOST}${CI_PROJECT_ID}" --header "PRIVATE-TOKEN:${GITLAB_PRIVATE_TOKEN}" | python3 -c "import sys, json; print(json.load(sys.stdin)['default_branch'])"`;
    # alternate branch read-out
...
##
# script to automatically create a merge-request
# adapteed from https://about.gitlab.com/2017/09/05/how-to-automatically-create-a-new-mr-on-gitlab-with-gitlab-ci/
##
